...
<%
	...
	if (idCuenta != 0) {
		...
		//si soy yo o soy admin o mas, informacion completa
		if (apodo.equals(session.getAttribute("apodo").toString()) || Integer.parseInt(basededatos
				.resConsultaSelectA3("SELECT idRole FROM accounts WHERE idAccount = " + idCuenta)[0][0]) <= 3) {
			autorizado = true;
		}
	} else {
...
<body>
	...
	<div class="container">
		...
		<div class="row text-center">
			...
			if (idCuenta != 0){
				tablares = basededatos.resConsultaSelectA3(
						"SELECT yo.nickname, yo.password, yo.email, yo.dateCreated, roles.description, staff.nickname, name, surname, birthdate, photo, idUser FROM accounts yo JOIN users ON (yo.idAccount = users.idAccount) JOIN accounts staff ON (yo.roleSetBy_idAccount = staff.idAccount) JOIN roles ON (roles.idRole = yo.idRole) WHERE yo.nickname LIKE '"
								+ apodo + "'");
			%>
			...
			<%
				if (tablares != null) {
			%>
			<a href="mandarmensaje.jsp?destinatario=<%=apodo%>"
				class="btn btn-default btn-sm">Mandar un mensaje</a>
			<%
				if (autorizado) {
			%><a class="btn btn-default"
				href="actualizarcuenta.jsp?apodo=<%=tablares[0][0]%>">Editar</a>
			<%
				}
			%>
			...
			<%
				if (tablares[0][9] != null && !tablares[0][9].equals("")) {
						fotoPlan = tablares[0][9];
					} else {
						fotoPlan = "img/user.png";
					}
			%>
			<img class="img-responsive img-thumbnail" src="<%=fotoPlan%>"
				width="100px" height="100px" />

			<p>
				<span class="text-info lead">Apodo:</span>
				<%=tablares[0][0]%></p>
			<%
				if (autorizado) {
			%>
			<p>
				<span class="text-info lead">Email:</span>
				<%=tablares[0][2]%></p>
			<%
				}
			%>
			<p>
				<span class="text-info lead">Cuenta creada el:</span>
				<%=tablares[0][3]%></p>
			<p>
				<span class="text-info lead">Rol:</span>
				<%=tablares[0][4]%></p>
			<%
				if (autorizado) {
			%>
			<p>
				<span class="text-info lead">Rol dado por:</span>
				<%=tablares[0][5]%></p>
			<%
				}
			%>
			...
			<h2>Personal:</h2>
			<p>
				<span class="text-info lead">Nombre:</span>
				<%=tablares[0][6]%></p>
			<p>
				<span class="text-info lead">Apellidos:</span>
				<%=tablares[0][7]%></p>
			<p>
				<span class="text-info lead">Birthday:</span>
				<%=tablares[0][8]%></p>

			<hr>
			<h2>Planes a los que asistira:</h2>
			<%
				tablares = basededatos.resConsultaSelectA3(
							"SELECT title, description, photo, idEvent FROM events WHERE idEvent IN(SELECT idEvent FROM userHasEvents WHERE idUser = "
									+ tablares[0][10] + ")");
					if (tablares != null) {
			...
			<div class="center-block" style="float: left;">
				<div class="thumbnail" style="max-width: 350px; height: 520px;">
					<img class="img-thumbnail" src="<%=fotoPlan%>" alt="<%=fotoPlan%>"
					...
					<div class="caption">
						...
						<p class="opcionesEvento">
							...
							<a href="plan.jsp?idPlan=<%=tablares[i][3]%>"
								class="btn btn-default" role="button">Mas detalles </a>
						</p>
					</div>
				</div>
			</div>
